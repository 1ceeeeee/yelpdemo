class RestaurauntsController < ApplicationController
  ...
  def show
    @reviews = Review.where(restauraunt_id: @restauraunt.id).order("created_at DESC")
    if @reviews.blank?
      @avgRating = 0
    else
      @avgRating = @reviews.average(:rating).round(2)
    end  
  end
  ...
  private
    ...
    def restauraunt_params
      params.require(:restauraunt).permit(:name, :address, :phone, :website, :image)
    end
end
